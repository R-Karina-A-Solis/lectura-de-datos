...
void leerArchivoCSV(const string& nombreArchivo, string*& encabezados, string**& datos, int& filas, int& columnas) {
    ...
    while (getline(archivo, linea)) {
        ...
        if (!esPrimeraLinea) {
            // Mostrar cada fila de datos leída
            ...
            // Mostrar los encabezados leídos
            cout << "Encabezados: ";
            for (int j = 0; j < columnas; ++j) {
...
// Función para realizar la búsqueda por salto (Jump Search)
void buscarClienteJumpSearch(string** datos, string* encabezados, int filas, int columnas, const string& clienteID) {
    ...
    sort(datos, datos + filas, [](const string* a, const string* b) {
        return a[0] < b[0]; // Suponiendo que el ID del cliente está en la primera columna
    });
    ...
    // Buscar el bloque en el que podría estar el cliente
    while (prev < filas && datos[min(filas, prev + salto)][0] < clienteID) {
        prev += salto;
        iteraciones++;
    }

    // Realizar búsqueda lineal en el bloque
